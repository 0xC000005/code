...
import cn.dataidea.tinylang.gen.TinyBaseVisitor;
...
import cn.dataidea.tinylang.vm.Assembler;
import cn.dataidea.tinylang.vm.Bytecode;
import cn.dataidea.tinylang.vm.Function;
import cn.dataidea.tinylang.vm.Label;

public class EmitPhase extends TinyBaseVisitor<ParseTree> {
	...
	public ParseTree visitIntVal(TinyParser.IntValContext ctx) {
		ParseTree body = visitChildren(ctx);
	...
	public ParseTree visitBoolVal(TinyParser.BoolValContext ctx) {
		ParseTree body = visitChildren(ctx);
	...
	public ParseTree visitPrintExp(TinyParser.PrintExpContext ctx) {
		ParseTree body = visitChildren(ctx);
		assem.print();
		return body;
	}

	public ParseTree visitFactor(TinyParser.FactorContext ctx) {
		ParseTree body = visitChildren(ctx);
	...
	public ParseTree visitTerm(TinyParser.TermContext ctx) {
		ParseTree body = visitChildren(ctx);
		Type right = types.get(ctx.right);
		if (right != null)
			assem.imul();
	...
	public ParseTree visitExp(TinyParser.ExpContext ctx) {
		ParseTree body = visitChildren(ctx);
		Type right = types.get(ctx.right);
		if (right != null)
			assem.iadd();
	...
	public ParseTree visitIfExp(TinyParser.IfExpContext ctx) {
	...
	public ParseTree visitGlobalVar(TinyParser.GlobalVarContext ctx) {
		TinyParser.VarasmContext varasm = ctx.varasm();
	...
	public ParseTree visitVarAssignment(TinyParser.VarAssignmentContext ctx) {
		TinyParser.VarasmContext varasm = ctx.varasm();
		String name = varasm.vardec().ID().getText();
	...
	public ParseTree visitVarExp(TinyParser.VarExpContext ctx) {
	...
	public ParseTree visitFunExp(TinyParser.FunExpContext ctx) {
		String name = ctx.ID().getText();
	...
	public ParseTree visitProg(TinyParser.ProgContext ctx) {
		ParseTree body = visitChildren(ctx);
		assem.halt();
	...
	public ParseTree visitDefcls(TinyParser.DefclsContext ctx) {
		...
		TinyParser.SlotsContext s = ctx.slots();
		if (s != null) {
			visit(ctx.slots());
		}
	...
	public ParseTree visitSupers(TinyParser.SupersContext ctx) {
	...
	public ParseTree visitSlotInit(TinyParser.SlotInitContext ctx) {
		// 1. get slot index
		TinyParser.VarasmContext varasm = ctx.varasm();
		String name = varasm.vardec().ID().getText();
	...
	public ParseTree visitClassExp(TinyParser.ClassExpContext ctx) {
	...
	public ParseTree visitMethodExp(TinyParser.MethodExpContext ctx) {
	...
	public ParseTree visitSlotExp(TinyParser.SlotExpContext ctx) {
		ValueContext object = ctx.value();
